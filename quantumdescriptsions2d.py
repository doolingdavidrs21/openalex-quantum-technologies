...
new intelligence analyst. Highlight any dual use technologies that may be of concern to the United States
Government.
...
def load_centroids_asat():
  ...
  #  df = pd.read_json(df.to_json())
    dg = pd.read_pickle('quantumcentroids2d.pkl.gz')
    #return dg
    return dg[dg.cluster != -1]

@st.cache_data()
def load_dftriple_asat():
    dg = pd.read_pickle('quantumdftriple2d.pkl.gz')
    return dg

@st.cache_data()
def load_dfinfo_asat():
    dg = pd.read_pickle('quantumdfinfo2d.pkl.gz')
    #return dg
    return dg[dg['cluster'] != -1]
